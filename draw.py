...
def draw_polygons( points, screen, color ):
    ...
    while point < len(points) -2:
        ...
        if cull( x0, y0, z0, x1, y1, z1, x2, y2, z2):
...
def add_sphere( edges, cx, cy, cz, r, step ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+1):
            ...
            x0 = points[index % plen][0]
            y0 = points[index % plen][1]
            z0 = points[index % plen][2]

            x1 = points[(index + 1) % plen][0]
            y1 = points[(index + 1) % plen][1]
            z1 = points[(index + 1) % plen][2]

            x2 = points[(index + num_steps) % plen][0]
            y2 = points[(index + num_steps) % plen][1]
            z2 = points[(index + num_steps) % plen][2]

            x3 = points[(index + num_steps + 1) % plen][0]
            y3 = points[(index + num_steps + 1) % plen][1]
            z3 = points[(index + num_steps + 1) % plen][2]

            add_polygon(edges, x0, y0, z0, x1, y1, z1, x3, y3, z3) #lower
            add_polygon(edges, x0, y0, z0, x3, y3, z3, x2, y2, z2) #upper
